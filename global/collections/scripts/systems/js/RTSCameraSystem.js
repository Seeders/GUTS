/**
 * RTSCameraSystem - RTS-style camera with game and third-person modes
 *
 * Features:
 * - Game camera mode with pan/zoom/rotate
 * - Third-person follow mode for units
 * - Entity following
 * - Position at player's starting buildings
 */
class RTSCameraSystem extends GUTS.BaseSystem {
  static services = [
    'getCamera',
    'setCamera',
    'cameraLookAt',
  ...
  /**
   * Get the zoom level - for RTS camera, always return 1.0 (fully zoomed out)
   * This ensures FogOfWar and other systems don't think we're in first-person mode
   */
  getZoomLevel() {
    return 1.0;
  }
  ...
  onSceneLoad(sceneData) {
    ...
    if (!this.activeCamera) {
      console.warn('[RTSCameraSystem] Camera not available in onSceneLoad - WorldSystem should call setCamera');
      return;
    }
