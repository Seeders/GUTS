{class PerformanceMonitor {
  ...
  getAverageTime(systemName, type = 'total') {
    const history = this.history.get(systemName);
    if (!history) return 0;
    const times = type === 'update' ? history.updateTimes : type === 'render' ? history.renderTimes : history.totalTimes;
  ...
  updateOverlay() {
    ...
    const sortedSystems = Array.from(this.systemStats.entries()).sort((a, b) => b[1].totalTime - a[1].totalTime);
  ...
  formatSystemName(systemName) {
    ...
    return systemName.replace(/System$/, '').replace(/([A-Z])/g, ' $1').trim();
  }
  reset() {
...
if (typeof window !== 'undefined' && window.GUTS) {
  window.GUTS.PerformanceMonitor = PerformanceMonitor;
}
