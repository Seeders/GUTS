...
function taxRateBps() {
    return parseInt(db.getSetting('tax_rate_bps', '0'), 10) || 0;
}
...
function createInvoiceFromUnbilled(clientId, opts = {}) {
    ...
    const termsDays = parseInt(db.getSetting('invoice_terms_days', '14'), 10) || 14;
...
function vaccinationAlerts() {
    const warnDays = parseInt(db.getSetting('vaccine_warn_days', '30'), 10) || 30;
    const required = db.getSetting('required_vaccines', 'rabies,dhpp,bordetella')
