...
adminRouter.put('/settings', (req, res) => {
    const allowed = [
        ...
        'tax_rate_bps', 'invoice_prefix', 'invoice_terms_days',
        'vaccine_warn_days', 'required_vaccines'
    ];
...
const BOOKING_STATUSES = ['requested', 'confirmed', 'checked_in', 'checked_out', 'cancelled'];
...
adminRouter.post('/bookings', (req, res, next) => {
    ...
    const id = db.tx(() => {
        const bookingId = db.insert('bookings', {
            ...
            check_in: checkIn,
            check_out: checkOut,
            status: BOOKING_STATUSES.includes(status) ? status : 'confirmed',
            source: 'admin',
            notes: clean(notes),
            created_at: stamp,
            updated_at: stamp
...
adminRouter.post('/bookings/:id/charges', (req, res, next) => {
    ...
    const service = db.get('SELECT * FROM services WHERE id = ?', serviceId);
...
adminRouter.post('/services', (req, res, next) => {
    ...
    if (!data.name) return next(fail(400, 'Service name is required.'));

    const id = db.insert('services', {
        ...data,
        unit: data.unit || 'each',
        price_cents: cents(req.body.price_cents),
        taxable: req.body.taxable === undefined ? 1 : bool(req.body.taxable),
...
adminRouter.put('/services/:id', (req, res, next) => {
    const id = Number(req.params.id);
    if (!db.get('SELECT id FROM services WHERE id = ?', id)) return next(fail(404, 'Service not found.'));

    const data = pick(req.body, SERVICE_FIELDS);
    if (req.body.price_cents !== undefined) data.price_cents = cents(req.body.price_cents);
    if (req.body.taxable !== undefined) data.taxable = bool(req.body.taxable);
    if (req.body.active !== undefined) data.active = bool(req.body.active);

    db.update('services', id, data);
    res.json(db.get('SELECT * FROM services WHERE id = ?', id));
});

/** Services are referenced by history, so retiring beats deleting. */
adminRouter.delete('/services/:id', (req, res) => {
    const id = Number(req.params.id);
    const used = db.get('SELECT COUNT(*) AS n FROM service_events WHERE service_id = ?', id).n;

    if (used > 0) {
        db.update('services', id, { active: 0 });
        return res.json({ ok: true, retired: true, message: 'Service retired (it appears in past charges).' });
    }
...
adminRouter.post('/service-events', (req, res, next) => {
    ...
    const service = req.body.service_id
        ? db.get('SELECT * FROM services WHERE id = ?', Number(req.body.service_id))
        : null;
...
const EXPENSE_CATEGORIES = [
    'food', 'supplies', 'cleaning', 'maintenance', 'utilities', 'rent',
    'insurance', 'payroll', 'marketing', 'vet', 'licensing', 'equipment',
    'software', 'fuel', 'other'
];
...
adminRouter.post('/expenses', upload.single('receipt'), (req, res, next) => {
    ...
    const id = db.insert('expenses', {
        ...
        is_maintenance: bool(req.body.is_maintenance) || (data.category === 'maintenance' ? 1 : 0),
        asset: data.asset,
