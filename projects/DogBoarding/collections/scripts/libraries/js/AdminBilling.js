...
class AdminBilling {
    ...
    unitLabel(unit) {
        return this.collections.serviceUnits?.[unit]?.title || this.ui.titleCase(unit);
    }

    methodLabel(method) {
        return this.collections.paymentMethods?.[method]?.title || this.ui.titleCase(method);
    }
            ...
            this.ui.mount(results,
                ...
                this.ui.table({
                    columns: [
                        ...
                        { label: 'Method', render: p => this.methodLabel(p.method) },
     ...
     *
     * It is the serviceCatalog collection, edited in the GUTS editor - there is
     * no copy of it in the database to edit here. A price change takes effect on
     * the next charge; it never rewrites what someone has already been billed,
     * because charges and invoices snapshot the price they were rung up at.
     */
    async servicesTab() {
        const { el } = this.ui;
        ...
        return el('div',
            el('div.notice',
                el('strong', 'This is the serviceCatalog collection. '),
                'Edit it in the GUTS editor and it takes effect here on the next refresh - ' +
                'no restart, no rebuild. Changing a price affects the next charge only; ' +
                'invoices already issued keep the price they were billed at.'),
            ...
            this.ui.table({
                columns: [
                    ...
                    { label: 'Collection object', render: s => el('code.muted', s.id) },
                    { label: 'Code', key: 'code' },
                    { label: 'Unit', render: s => this.unitLabel(s.unit) },
                ...
                rows: services,
                empty: 'The serviceCatalog collection is empty.'
            }));
    }
        ...
        const body = el('div',
            ...
            el('div.field-row',
                ...
                this.ui.field({ name: 'paid_on', label: 'Received on', type: 'date',
                    value: this.ui.today() }),
                // Payment methods are a collection, so adding one is an editor edit.
                this.ui.field({ name: 'method', label: 'Method', type: 'select',
                    options: this.ui.ordered(this.collections.paymentMethods)
        ...
        this.ui.modal({
            ...
            onConfirm: async (content) => {
                ...
                await this.api.createServiceEvent({
                    ...
                    service_id: serviceSelect.value || null,   // the collection object id
