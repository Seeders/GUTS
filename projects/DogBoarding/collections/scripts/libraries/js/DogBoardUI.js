...
class DogBoardUI {
    ...
    /* ---------------- binding markup to content ---------------- */

    /** Walk a dotted path: get({a:{b:1}}, 'a.b') -> 1 */
    static get(obj, path) {
        return String(path).split('.').reduce((acc, key) => (acc == null ? acc : acc[key]), obj);
    }

    /**
     * Fill an interface's markup from the content collections.
     *
     *   <h1 data-bind="home.hero.headline">     -> textContent
     *   <a  data-cta="home.hero.primaryCta">    -> label + href from { label, route }
     *   <a  data-tel data-bind="business.phone">-> also sets href="tel:…"
     *
     * This is why the HTML lives in the interfaces collection: the markup says
     * where a thing goes, the content collection says what it says.
     */
    static bind(root, data) {
        root.querySelectorAll('[data-bind]').forEach(node => {
            const value = DogBoardUI.get(data, node.dataset.bind);
            if (value === undefined || value === null) return;

            node.textContent = String(value);

            if (node.hasAttribute('data-tel')) node.setAttribute('href', `tel:${value}`);
            if (node.hasAttribute('data-mailto')) node.setAttribute('href', `mailto:${value}`);
        });

        root.querySelectorAll('[data-cta]').forEach(node => {
            const cta = DogBoardUI.get(data, node.dataset.cta);
            if (!cta) return;

            node.textContent = cta.label || '';
            if (cta.route) node.setAttribute('href', `#${cta.route}`);
        });

        return root;
    }

    /** Collection -> array, ordered by `order`, with the object's key as `id`. */
    static ordered(collection) {
    ...
    static modal({ title, body, confirmLabel = 'Save', cancelLabel = 'Cancel',
        ...
        // The modal chrome is a <template> in the interface collection. Only fall
        // back to building it here if the current interface has no template.
        const tpl = document.querySelector('template[data-template="modal"]');

        const overlay = tpl
            ? tpl.content.firstElementChild.cloneNode(true)
            : DogBoardUI.el('div.modal-overlay',
                DogBoardUI.el('div.modal',
                    DogBoardUI.el('header.modal__head',
                        DogBoardUI.el('h2', { dataset: { modalTitle: '' } }),
                        DogBoardUI.el('button.modal__x', { type: 'button', dataset: { modalClose: '' } }, '×')),
                    DogBoardUI.el('div.modal__body', { dataset: { modalBody: '' } }),
                    DogBoardUI.el('footer.modal__foot',
                        DogBoardUI.el('button.btn', { type: 'button', dataset: { modalCancel: '' } }),
                        DogBoardUI.el('button.btn.btn--primary', { type: 'button', dataset: { modalConfirm: '' } }))));

        const panel = overlay.querySelector('.modal');
        panel.style.maxWidth = `${width}px`;

        overlay.querySelector('[data-modal-title]').textContent = title;
        DogBoardUI.mount(overlay.querySelector('[data-modal-body]'), content);
