...
class AdminBookings {
    ...
    /** Label + tone for a booking status, from the bookingStatuses collection. */
    statusBadge(status) {
    ...
    async list() {
        ...
        // Every status this business has is a bookingStatuses object.
        const statusSelect = el('select', { onchange: () => load() },
            [el('option', { value: '' }, 'All bookings'),
             ...this.ui.ordered(this.collections.bookingStatuses).map(s =>
                el('option', { value: s.id }, s.title))]);
        ...
        const load = async () => {
            ...
            this.ui.mount(results, this.ui.table({
                columns: [
                    ...
                    { label: 'Status', render: b => this.statusBadge(b.status) },
    ...
    /**
     * Which buttons a booking offers is not decided here. Each bookingStatuses
     * object lists its own `actions`: where it can go, and what to call the
     * button. Add a status in the editor and it wires itself up.
     */
    statusActions(booking) {
        const { el } = this.ui;
        const def = this.collections.bookingStatuses?.[booking.status];
        const actions = def?.actions || [];

        return actions.map(action => el('button.btn', {
            class: action.primary ? 'btn--primary' : '',
            onclick: async () => {
                try {
                    await this.api.updateBooking(booking.id, { status: action.to });
                    const target = this.collections.bookingStatuses?.[action.to];
                    this.ui.toast(`Booking: ${target?.title || action.to}.`, 'good');
                    this.reload();
                } catch (err) {
                    this.ui.toast(err.message, 'bad');
                }
    ...
    editBooking(booking) {
        ...
        const body = el('div',
            this.forms.renderFields('booking', booking),
        ...
        this.ui.modal({
            ...
            confirmLabel: 'Save',
            onConfirm: async (content) => {
                await this.api.updateBooking(booking.id, this.forms.read('booking', content));
    ...
    async addBoardingCharges(booking) {
        ...
        const updatePreview = () => {
            ...
            // Whether a service multiplies by nights is a property of its unit.
            const perNight = !!this.collections.serviceUnits?.[service.unit]?.perNight;
        ...
        this.ui.modal({
            ...
            onConfirm: async (content) => {
                ...
                const result = await this.api.addBookingCharges(booking.id, {
                    service_id: serviceSelect.value,   // the collection object id
                    pet_ids: petIds
                });
