...
class AdminClients {
    ...
    recordsTable(pet) {
        ...
        return this.ui.table({
            columns: [
                { label: 'Type', render: r => el('strong', this.ui.titleCase(r.record_type)) },
        ...
        return el('div',
            ...
            this.ui.row(
                this.ui.field({ name: 'email', label: 'Email', type: 'email', required: true,
                    value: client.email }),
                this.ui.field({ name: 'phone', label: 'Phone', required: true, value: client.phone }),
                this.ui.field({ name: 'alt_phone', label: 'Alt phone', value: client.alt_phone })),
            this.ui.field({ name: 'address1', label: 'Address', value: client.address1 }),
            this.ui.field({ name: 'address2', label: 'Address line 2', value: client.address2 }),
            this.ui.row(
                this.ui.field({ name: 'city', label: 'City', value: client.city }),
                this.ui.field({ name: 'state', label: 'State', value: client.state }),
                this.ui.field({ name: 'postal_code', label: 'ZIP', value: client.postal_code })),
            this.ui.row(
                this.ui.field({ name: 'emergency_name', label: 'Emergency contact',
                    value: client.emergency_name }),
                this.ui.field({ name: 'emergency_phone', label: 'Emergency phone',
                    value: client.emergency_phone }),
                this.ui.field({ name: 'emergency_relationship', label: 'Relationship',
                    value: client.emergency_relationship })),
            this.ui.field({ name: 'status', label: 'Status', type: 'select',
                value: client.status || 'active',
                options: ['pending', 'active', 'archived'] }),
            this.ui.field({ name: 'notes', label: 'Internal notes', type: 'textarea',
                value: client.notes,
    ...
    petFields(pet = {}, client) {
        const { el } = this.ui;
        const vetOptions = [{ value: '', label: '— none —' },
            ...client.vets.map(v => ({ value: v.id, label: v.clinic_name }))];

        return el('div',
            this.ui.row(
                this.ui.field({ name: 'name', label: 'Name', required: true, value: pet.name }),
                this.ui.field({ name: 'breed', label: 'Breed', value: pet.breed })),
            this.ui.row(
                this.ui.field({ name: 'sex', label: 'Sex', type: 'select', value: pet.sex,
                ...
                this.ui.field({ name: 'weight_lbs', label: 'Weight (lbs)', type: 'number',
                    step: '0.1', value: pet.weight_lbs })),
            this.ui.row(
                this.ui.field({ name: 'color', label: 'Colour', value: pet.color }),
                this.ui.field({ name: 'microchip', label: 'Microchip', value: pet.microchip })),
            this.ui.row(
                this.ui.field({ name: 'vet_id', label: 'Veterinarian', type: 'select',
                    value: pet.vet_id, options: vetOptions }),
                this.ui.field({ name: 'status', label: 'Status', type: 'select',
                    value: pet.status || 'active', options: ['active', 'inactive', 'deceased'] })),
            this.ui.field({ name: 'fixed', label: 'Spayed or neutered', type: 'checkbox',
                value: pet.fixed }),
            this.ui.field({ name: 'feeding', label: 'Feeding', type: 'textarea', value: pet.feeding }),
            this.ui.field({ name: 'medications', label: 'Medications', type: 'textarea',
                value: pet.medications }),
            this.ui.field({ name: 'allergies', label: 'Allergies', type: 'textarea',
                value: pet.allergies }),
            this.ui.field({ name: 'behavior_notes', label: 'Behaviour', type: 'textarea',
                value: pet.behavior_notes }),
            this.ui.field({ name: 'vet_notes', label: 'Vet notes', type: 'textarea',
                value: pet.vet_notes }));
    }
    ...
    addRecord(pet) {
        ...
        const body = el('div',
            this.ui.row(
                this.ui.field({ name: 'record_type', label: 'Record type', type: 'select',
                    options: ['rabies', 'dhpp', 'bordetella', 'influenza', 'leptospirosis',
                        'exam', 'titer', 'other'] }),
                this.ui.field({ name: 'issued_on', label: 'Issued', type: 'date' }),
                this.ui.field({ name: 'expires_on', label: 'Expires', type: 'date' })),
            el('label.field',
                el('span.field__label', 'Document (PDF or photo)'),
                el('input', { type: 'file', name: 'file', accept: 'application/pdf,image/*' })),
            this.ui.field({ name: 'verified', label: 'Mark as verified', type: 'checkbox',
                hint: 'Only tick this if you have actually looked at the certificate.' }),
    ...
    async deleteRecord(record) {
        const ok = await this.ui.confirm(
            `Delete this ${this.ui.titleCase(record.record_type)} record and its document?`,
            { title: 'Delete record' });
        if (!ok) return;
    ...
    vetFields(vet = {}) {
        ...
        return el('div',
            this.ui.row(
                this.ui.field({ name: 'clinic_name', label: 'Clinic name', required: true,
                    value: vet.clinic_name }),
                this.ui.field({ name: 'vet_name', label: 'Veterinarian', value: vet.vet_name })),
            this.ui.row(
                this.ui.field({ name: 'phone', label: 'Phone', value: vet.phone }),
                this.ui.field({ name: 'email', label: 'Email', type: 'email', value: vet.email })),
            this.ui.field({ name: 'address1', label: 'Address', value: vet.address1 }),
            this.ui.row(
                this.ui.field({ name: 'city', label: 'City', value: vet.city }),
                this.ui.field({ name: 'state', label: 'State', value: vet.state }),
                this.ui.field({ name: 'postal_code', label: 'ZIP', value: vet.postal_code })),
            this.ui.field({ name: 'notes', label: 'Notes', type: 'textarea', value: vet.notes }));
    }
