/**
 * CommandQueueSystem - Manages unit command queues and AI controller switching
 *
 * PROBLEM SOLVED:
 * - Units no longer finish old paths before executing new commands (e.g., build orders)
 * - Paths are properly cleared when switching between AI controllers
 * - Commands have priorities to prevent unwanted interruptions
 * - Client-server sync issues resolved by consistent command handling
 *
 * USAGE:
 *
 * 1. Queue a command:
 *    this.game.gameManager.call('queueCommand', unitId, {
 *        type: 'build',                          // Command type
 *        controllerId: 'buildingState',          // AI controller to activate
 *        targetPosition: { x, y, z },            // Target position
 *        target: buildingEntityId,               // Target entity (optional)
 ...
 *    }, true); // true = interrupt current command if priority is higher
 *
 * 2. Complete a command:
 *    this.game.gameManager.call('completeCurrentCommand', unitId);
 *
 * 3. Clear all commands:
 *    this.game.gameManager.call('clearCommands', unitId);
 *
 * PRIORITY LEVELS:
 * - IDLE (0): Automatic behaviors like mining, combat AI
 * - MOVE (10): Player movement commands
 * - BUILD (20): Building construction
 * - ATTACK (30): Explicit attack commands
 * - ABILITY (40): Special abilities
 * - FORCED (100): Cannot be interrupted
 *
 * Higher priority commands interrupt lower priority ones (if interruptible = true)
 */
