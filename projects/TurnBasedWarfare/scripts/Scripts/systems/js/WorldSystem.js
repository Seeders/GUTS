class WorldSystem extends engine.BaseSystem {
    ...
    simplifyGroundMesh() {
        if (!this.ground || !this.heightMapData) return;
        ...
        // Analyze terrain to find flat regions
        const chunkSize = 8; // Analyze in 8x8 vertex chunks
        const flatThreshold = this.heightStep * 0.5; // If height variance is less than half a height step, consider it flat
        const chunksPerRow = Math.ceil(segments / chunkSize);

        const flatMap = new Array(chunksPerRow * chunksPerRow).fill(false);

        // Analyze each chunk for flatness
        for (let chunkZ = 0; chunkZ < chunksPerRow; chunkZ++) {
            for (let chunkX = 0; chunkX < chunksPerRow; chunkX++) {
                const startX = chunkX * chunkSize;
                const startZ = chunkZ * chunkSize;
                const endX = Math.min(startX + chunkSize + 1, verticesPerRow);
                const endZ = Math.min(startZ + chunkSize + 1, verticesPerRow);

                let minHeight = Infinity;
                let maxHeight = -Infinity;

                // Sample vertices in this chunk
                for (let z = startZ; z < endZ; z++) {
                    for (let x = startX; x < endX; x++) {
                        const idx = (z * verticesPerRow + x) * 3;
                ...
                const chunkIndex = chunkZ * chunksPerRow + chunkX;
                flatMap[chunkIndex] = variance < flatThreshold;
            }
        }

        // Count flat chunks to estimate memory savings
        const flatChunkCount = flatMap.filter(f => f).length;
        const flatPercentage = (flatChunkCount / flatMap.length) * 100;

        console.log(`Ground mesh analysis: ${flatChunkCount}/${flatMap.length} chunks (${flatPercentage.toFixed(1)}%) are flat`);

        // If more than 30% of terrain is flat, rebuild with optimized geometry
        if (flatPercentage > 30) {
            this.rebuildOptimizedGroundMesh(flatMap, chunksPerRow, chunkSize);
        }
    }

    rebuildOptimizedGroundMesh(flatMap, chunksPerRow, chunkSize) {
        const originalGeometry = this.ground.geometry;
        const segments = this.heightMapResolution;

        // Build new geometry with variable resolution
        const newVertices = [];
        const newUVs = [];
        const newIndices = [];
        const vertexMap = new Map(); // Maps original vertex key to new vertex index

        const getOrCreateVertex = (x, z) => {
            const key = `${x},${z}`;
            ...
            const worldX = (nx - 0.5) * this.extendedSize;
            const worldZ = (nz - 0.5) * this.extendedSize;

            const vertexIndex = newVertices.length / 3;
            newVertices.push(worldX, worldZ, height);
            newUVs.push(nx, nz);
        ...
        // Build triangles with adaptive detail
        for (let chunkZ = 0; chunkZ < chunksPerRow; chunkZ++) {
            for (let chunkX = 0; chunkX < chunksPerRow; chunkX++) {
                const chunkIndex = chunkZ * chunksPerRow + chunkX;
                const isFlat = flatMap[chunkIndex];

                const startX = chunkX * chunkSize;
                const startZ = chunkZ * chunkSize;
                const endX = Math.min(startX + chunkSize, segments);
                const endZ = Math.min(startZ + chunkSize, segments);

                // Use larger steps for flat areas
                const step = isFlat ? Math.max(2, Math.floor(chunkSize / 2)) : 1;

                for (let z = startZ; z < endZ; z += step) {
                    for (let x = startX; x < endX; x += step) {
                        const nextX = Math.min(x + step, segments);
                        const nextZ = Math.min(z + step, segments);

                        // Create two triangles for this quad
                        const v0 = getOrCreateVertex(x, z);
                        const v1 = getOrCreateVertex(nextX, z);
                        const v2 = getOrCreateVertex(nextX, nextZ);
                        const v3 = getOrCreateVertex(x, nextZ);
        ...
        // Calculate reduction
        const originalTriangles = segments * segments * 2;
        const newTriangles = newIndices.length / 3;
        const reduction = ((1 - newTriangles / originalTriangles) * 100).toFixed(1);

        console.log(`Optimized ground mesh: ${newTriangles} triangles (${reduction}% reduction from ${originalTriangles})`);

        // Replace geometry
        originalGeometry.dispose();
        this.ground.geometry = newGeometry;
        this.groundVertices = newGeometry.attributes.position;
    }
