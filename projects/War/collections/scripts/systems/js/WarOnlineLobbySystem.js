/**
 * WarOnlineLobbySystem - Manages the online lobby UI
 * Shows chat lobby as main view with Create/Join game options
 */
class WarOnlineLobbySystem extends GUTS.BaseSystem {
    static services = [
        'refreshGamesList',
        'showOnlineLobby',
        'hideOnlineLobby',
    ...
    async updateLobbyStats() {
        try {
            ...
            if (statsResponse.ok) {
                ...
                if (countEl) countEl.textContent = stats.connectedPlayers || 0;
            }

            const roomsResponse = await fetch('/api/rooms');
            if (roomsResponse.ok) {
                const rooms = await roomsResponse.json();
                const availableGames = rooms.filter(r =>
                    r.playerCount > 0 && r.playerCount < r.maxPlayers && !r.isActive
                );
                const activeGames = rooms.filter(r => r.isActive);

                const availableEl = document.getElementById('availableGamesCount');
                const activeEl = document.getElementById('activeGamesCount');
                if (availableEl) availableEl.textContent = availableGames.length;
                if (activeEl) activeEl.textContent = activeGames.length;
            }
        } catch (error) {
            // Silently fail
        }
        ...
        try {
            ...
            container.querySelectorAll('.join-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    this.joinGame(e.target.dataset.roomId);
                });
            });
        } catch (error) {
    ...
    createGame() {
        const playerName = this.game.state.playerName || 'Player';
        this.call.createRoom(playerName);
    }
