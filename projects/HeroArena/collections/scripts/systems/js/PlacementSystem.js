...
class PlacementSystem extends GUTS.BaseSystem {
    ...
    spawnStartingUnitsForTeam(team) {
        console.log('[PlacementSystem] spawnStartingUnitsForTeam called for team:', team);
        const startingUnitsConfig = this.collections.configs.startingUnits;
     ...
     */
    // HeroArena: spawn an array of hero unit spawnTypes for a given team.
    // spawnTypes is an array of unit spawnType strings, e.g. ['1_s_barbarian', '1_i_apprentice'].
    spawnHeroUnitsForTeam(spawnTypes, team) {
        const startingLocations = this.getStartingLocationsFromLevel();
        if (!startingLocations || !startingLocations[team]) {
            console.error('[PlacementSystem] spawnHeroUnitsForTeam: no starting location for team', team);
    ...
    *_spiralOffsets(maxRing) {
    ...
    _spawnStartingUnitsForTeamInternal(prefabs, team, startingWorldPos, goldVeinPos = null) {
        ...
        console.log('[PlacementSystem] Spawned', spawnedUnits.length, 'starting units for team', team);
        return {
            team: team,
