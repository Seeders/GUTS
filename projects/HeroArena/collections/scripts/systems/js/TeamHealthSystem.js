class TeamHealthSystem extends GUTS.BaseSystem {
    ...
    init() {
        // HeroArena: HP lives on playerStats.hp (100 max).
        // This system still manages the UI health bar display.
        this.MAX_TEAM_HEALTH = 100;
    ...
    // Called by ClientNetworkSystem after entity sync so the HP bar stays current.
    syncFromPlayerStats() {
        const entities = this.game.getEntitiesWith('playerStats');
        for (const entityId of entities) {
            const stats = this.game.getComponent(entityId, 'playerStats');
            if (!stats) continue;
            const hp = stats.hp ?? 100;
            this.teamHealth[stats.team] = hp;
        }
        this.updateHealthDisplay();
    }
