class AbilitySystem extends GUTS.BaseSystem {
        ...
        for (const entityId of this._sortedQueueEntities) {
            ...
            if (this.game.state.now >= queuedAbility.executeTime) {
                ...
                if (abilities) {
                    ...
                    if (ability) {
                        // Reconstruct targetData from flattened fields
                        let targetData = queuedAbility.targetData;

                        // If there's a stored target position, reconstruct the params object
                        if (queuedAbility.hasTargetPosition) {
                            targetData = {
                                targetPosition: {
                                    x: queuedAbility.targetPositionX,
                                    y: queuedAbility.targetPositionY,
                                    z: queuedAbility.targetPositionZ
                                }
                            };
                        }

                        console.log(`[AbilitySystem] Executing ${abilityName} with targetData:`, targetData);
    ...
    considerAbilityUsage(entityId, abilities) {
        // Skip player-controlled entities - they use abilities manually
    ...
    useAbility(entityId, abilityId, targetData = null) {
        ...
        if (existingQueue && (existingQueue.abilityId != null || existingQueue.abilityStringId)) {
            return false;
        }

        const abilities = this.entityAbilities.get(entityId);
        if (!abilities) {
            console.log(`[AbilitySystem] useAbility failed: entity ${entityId} has no abilities registered`);
            return false;
        }

        const ability = abilities.find(a => a.id === abilityId);
        if (!ability) {
            console.log(`[AbilitySystem] useAbility failed: ability '${abilityId}' not found for entity ${entityId}. Available: ${abilities.map(a => a.id).join(', ')}`);
            return false;
        }
        ...
        const deathState = this.game.getComponent(entityId, "deathState");
        if (deathState && deathState.state !== this.enums.deathState.alive) {
            console.log(`[AbilitySystem] useAbility failed: entity ${entityId} is dead/dying`);
            return false;
        }

        if (!this.isAbilityOffCooldown(entityId, abilityId)) {
            console.log(`[AbilitySystem] useAbility failed: ability '${abilityId}' is on cooldown`);
            return false;
        }

        if (!ability.canExecute(entityId, targetData)) {
            console.log(`[AbilitySystem] useAbility failed: ability '${abilityId}' canExecute returned false`);
            return false;
        }
        ...
        // Extract targetPosition if present (for position-targeted abilities)
        // Complex objects can't be stored in TypedArray components, so flatten it
        const targetPosition = targetData?.targetPosition;
        const hasTargetPosition = targetPosition ? 1 : 0;
        ...
        this.game.addComponent(entityId, 'abilityQueue', {
            ...
            targetData: typeof targetData === 'number' ? targetData : null,
    ...
    onSceneUnload() {
        // Clear all ability instances - they'll be recreated when new scene loads
