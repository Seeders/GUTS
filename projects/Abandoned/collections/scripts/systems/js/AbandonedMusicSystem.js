...
/**
 * AbandonedMusicSystem - Music system for the Abandoned survival card game
 * Extends the global MusicSystem to add game-specific music behaviors
 */
class AbandonedMusicSystem extends MusicSystem {
    static services = [
    ...
    loadMusicPreference() {
        try {
            const saved = localStorage.getItem('abandonedMusicEnabled');
            if (saved !== null) {
                this.musicEnabled = saved === 'true';
            }
            const savedVolume = localStorage.getItem('abandonedMusicVolume');
            if (savedVolume !== null) {
    ...
    saveMusicPreference() {
        try {
            localStorage.setItem('abandonedMusicEnabled', this.musicEnabled.toString());
            localStorage.setItem('abandonedMusicVolume', this.musicVolume.toString());
        } catch (e) {
            // Ignore localStorage errors
        }
    }
    ...
    async startGameMusic() {
        ...
        // Play the war track (dark, tense music fits survival theme)
...
export default AbandonedMusicSystem;
