class DashAbility extends GUTS.BaseAbility {
    constructor(game, abilityData = {}) {
        super(game, abilityData);
        this.dashDistance = abilityData.dashDistance ?? 140;
    ...
    execute(casterEntity) {
        ...
        this.logAbilityUsage(casterEntity, "Dashes through enemies!");

        const dx = tp.x - pos.x;
        const dz = tp.z - pos.z;
        const d  = Math.sqrt(dx * dx + dz * dz);
        if (d < 0.001) return;
        const step = Math.min(this.dashDistance, d - 10);
        pos.x += (dx / d) * step;
        pos.z += (dz / d) * step;

        // Damage everything in a small radius around new position
        const hits = this.getEnemiesInRange(casterEntity, 60);
        hits.forEach(eid => {
            this.dealDamageWithEffects(casterEntity, eid, this.damage, this.element, { isMelee: true });
