...
function resolveTerrainCollision(entity, deltaTime, getHeight, x, z) {
    // Check if entity is colliding with terrain
    const heightAtPoint = getHeight(x, z);
    ...
    if (entity.position.y - sampleDistance <= heightAtPoint) {
        ...
        const heightAtPointPlusX = getHeight(x + sampleDistance, z);
        const heightAtPointPlusZ = getHeight(x, z + sampleDistance);
...
self.onmessage = function(e) {
    ...
    if (!terrainGenerator && biomeConfig) {
        ...
        terrainGenerator.init(biomeConfig, chunkSize, chunkResolution);
    } else if (biomeConfig) {
    ...
    entities.forEach(entity => {
        ...
        if (terrainGenerator) {
            ...
            resolveTerrainCollision(entity, deltaTime, terrainGenerator.getHeight.bind(terrainGenerator), x, z);
        }
